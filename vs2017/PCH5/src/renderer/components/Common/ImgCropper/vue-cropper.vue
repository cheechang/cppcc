<template>
  <div class="vue-cropper" ref="cropper">
    ...
    >
      <span class="cropper-face cropper-move" @mousemove="$emit('changeCropEnd')" @mousedown="cropMove"></span>
      <span class="crop-info" v-if="info" :style="{'top': cropInfo}">{{ this.cropW }} × {{ this.cropH }}</span>
      <span class="crop-line line-w" @mousedown="changeCropSize($event, false, true, 0, 1)" @mousemove="$emit('changeCropEnd')"></span>
      <span class="crop-line line-a" @mousedown="changeCropSize($event, true, false, 1, 0)" @mousemove="$emit('changeCropEnd')"></span>
      <span class="crop-line line-s" @mousedown="changeCropSize($event, false, true, 0, 2)" @mousemove="$emit('changeCropEnd')"></span>
      <span class="crop-line line-d" @mousedown="changeCropSize($event, true, false, 2, 0)" @mousemove="$emit('changeCropEnd')"></span>
      <span class="crop-point point1" @mousedown="changeCropSize($event, true, true, 1, 1)" @mousemove="$emit('changeCropEnd')"></span>
      <span class="crop-point point2" @mousedown="changeCropSize($event, false, true, 0, 1)" @mousemove="$emit('changeCropEnd')"></span>
      <span class="crop-point point3" @mousedown="changeCropSize($event, true, true, 2, 1)" @mousemove="$emit('changeCropEnd')"></span>
      <span class="crop-point point4" @mousedown="changeCropSize($event, true, false, 1, 0)" @mousemove="$emit('changeCropEnd')"></span>
      <span class="crop-point point5" @mousedown="changeCropSize($event, true, false, 2, 0)" @mousemove="$emit('changeCropEnd')"></span>
      <span class="crop-point point6" @mousedown="changeCropSize($event, true, true, 1, 2)" @mousemove="$emit('changeCropEnd')"></span>
      <span class="crop-point point7" @mousedown="changeCropSize($event, false, true, 0, 2)" @mousemove="$emit('changeCropEnd')"></span>
      <span class="crop-point point8" @mousedown="changeCropSize($event, true, true, 2, 2)" @mousemove="$emit('changeCropEnd')"></span>
    </div>
  </div>
</template>
    ...
    getCropData(cb) {
      ...
      img.onload = () => {
        ...
        if (~~canvas.width !== 0) {
          // 图片x轴偏移
          let dx =
            this.x - this.cropOffsertX + this.trueWidth * (1 - this.scale) / 2;
          // 图片y轴偏移
          let dy =
            this.y - this.cropOffsertY + this.trueHeight * (1 - this.scale) / 2;
          // console.log(dx, dy)
          ctx.drawImage(
            img,
            dx,
