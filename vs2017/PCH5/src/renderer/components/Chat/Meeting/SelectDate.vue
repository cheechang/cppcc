<template>
  <div class='edit-time-components'>
    <div :id='index' class='time-box' v-for='(item,index) in dateList' :key="index">
      <div class='year-month-box'>
        ...
        <div v-if='item.switch' class='year-list-box'>
          ...
          <ul v-if='showYearList === index' @mousewheel='mousewheel($event)' class='year-list'>
            <li v-for='y in 25' :key="'25'+y" @click='changeYearMonth(2000 + y, item.month, index)' class='year-item'>{{2000 + y}}年</li>
          </ul>
          <ul v-if='showMonthList === index' @mousewheel='mousewheel($event)' class='month-list'>
            <li v-for='m in 12' :key="'12'+m" @click='changeYearMonth(item.year, m, index)' class='month-item'>{{m}}月</li>
          </ul>
        </div>
      ...
      <ul class='ul-box'>
        <li class='li-item week'>日</li>
        <li class='li-item week'>一</li>
        <li class='li-item week'>二</li>
        <li class='li-item week'>三</li>
        <li class='li-item week'>四</li>
        <li class='li-item week'>五</li>
        <li class='li-item week'>六</li>
      </ul>
      <ul class='ul-box'>
        <li class='li-item' v-for='i in item.week' :key="'week'+i"></li>
        <li
          ...
          :style = '{
            background: (selected.has(formatDate(item.year, item.month, i)) ? "#5bc0de" : ""),
            color: (selected.has(formatDate(item.year, item.month, i)) ? "#fff" : "")
          }'
          :select = '(selected.has(formatDate(item.year, item.month, i)) ? "true" : "false")'
          :data-index = 'i + 1'
          @click='dateClick($event, item, i)'
          @dblclick='dbDateClick($event, item, i)'
...
export default {
  ...
  methods: {
    ...
    changeYearMonth(year, month, index) {
      ...
      this.$set(this.dateList, index, date);
    },
    ...
    dateClick(event, item, day) {
      console.log('item11111',item,day)
