...
#include "../common/Common.h"
 namespace ui
 {
	 ThirdVerifyWidget::ThirdVerifyWidget()
	 {
		...
		// m_strAccount = account;
		 ...
		 if (m_authcontrol)
		 {
			// m_authcontrol->verifyImgCode(account,"",CBind(&ThirdVerifyWidget::signalSerGetVerifyCode,this,std::placeholders::_1,std::placeholders::_2));
		 }
		 this->show();
		 this->raise();
	 }
	 ...
	 ThirdVerifyWidget::~ThirdVerifyWidget(void)
	 {
	 }

	 void ThirdVerifyWidget::InitWidgets()
	 {
	 ...
	 void ThirdVerifyWidget::onSerGetNextImage()
	 {
		 if (m_authcontrol)
		 {
			 m_authcontrol->verifyImgCode(m_strAccount,"",CBind(&ThirdVerifyWidget::signalSerGetVerifyCode,this,std::placeholders::_1,std::placeholders::_2));
		 }
	 }


	 void ThirdVerifyWidget::onSerGetVerifyCode( int err, const std::string codeUrl )
	 {
		  if (err == 0)
		  {
	 ...
	 void ThirdVerifyWidget::DoVerifyCode( std::string account )
	 {

	 }

	 void ThirdVerifyWidget::onBtnLockOutClicekd()
	 {
		 std::string code = m_editCodeInput->text().toUtf8().data();
		 if (m_authcontrol)
		 {
			 m_authcontrol->verifyImgCode(m_strAccount,code,CBind(&ThirdVerifyWidget::signalDoLockoutResult,this,std::placeholders::_1,std::placeholders::_2));
		 }
	 }

	 void ThirdVerifyWidget::onSerLockoutResult( int err, const std::string codeUrl )
	 {
		if (err == 0)
		{
			emit signalLockout();
	 ...
	 void ThirdVerifyWidget::Setdata(QByteArray imagContent)
	 {
		 QImage image;  
		 image.loadFromData((const uchar*)imagContent.data(), imagContent.size());  
		  
		  m_labCodeImage->setPixmap(QPixmap::fromImage(image));
	 }
