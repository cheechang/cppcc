...
namespace uicontrols
{
	...
	void CircleProgressBar::setMinimum(double min)
	{
		setRange(min, m_max);
	}

	void CircleProgressBar::setMaximum(double max)
	{
		setRange(m_min, max);
	}

	void CircleProgressBar::setValue(double val)
	{
		if (m_value != val)
		{
			if (val < m_min)
	...
	void CircleProgressBar::setValue(int val)
	{
		setValue(static_cast<double>(val));
	}

	void CircleProgressBar::setNullPosition(double position)
	{
	...
	void CircleProgressBar::setBarStyle(BarStyle style)
	{
		if (style != m_barStyle)
		{
			m_barStyle = style;
	...
	void CircleProgressBar::setOutlinePenWidth(double penWidth)
	{
	...
	void CircleProgressBar::setDataPenWidth(double penWidth)
	{
	...
	void CircleProgressBar::setDataColors(const QGradientStops& stopPoints)
	{
	...
	void CircleProgressBar::setFormat(const QString& format)
	{
		if (format != m_format)
		{
			m_format = format;
	...
	void CircleProgressBar::resetFormat()
	{
	...
	void CircleProgressBar::setDecimals(int count)
	{
	...
	void CircleProgressBar::drawBackground(QPainter& p, const QRectF& baseRect)
	{
		p.fillRect(baseRect, palette().background());
	}

	void CircleProgressBar::drawBase(QPainter& p, const QRectF& baseRect, const QRectF& innerRect)
	{
		switch (m_barStyle)
		{
		case StyleDonut:
	...
	void CircleProgressBar::drawValue(QPainter& p
	...
	void CircleProgressBar::calculateInnerRect(const QRectF&/*baseRect*/, double outerRadius, QRectF& innerRect,
	                                           double& innerRadius)
	{
		// for Line style
		if (m_barStyle == StyleLine)
	...
	void CircleProgressBar::drawInnerBackground(QPainter& p, const QRectF& innerRect)
	{
		if (m_barStyle == StyleDonut)
		{
	...
	void CircleProgressBar::drawText(QPainter& p, const QRectF& innerRect, double innerRadius, double value)
	{
		if (m_format.isEmpty())
			return;
		...
		// !!! to revise
		QFont f(font());
		f.setPointSize(innerRadius * qMax(0.05, (0.35 - static_cast<double>(m_decimals) * 0.08)));
	...
	QString CircleProgressBar::valueToText(double value) const
	{
	...
	void CircleProgressBar::valueFormatChanged()
	{
		m_updateFlags = 0;
	...
	void CircleProgressBar::rebuildDataBrushIfNeeded()
	{
		if (m_rebuildBrush)
		{
			m_rebuildBrush = false;
	...
	float CircleProgressBar::innerOuterRate() const
	{
		return m_innerOuterRate;
	}

	void CircleProgressBar::setInnerOuterRate(float innerOuterRate)
	{
