...
int Rijndael::padEncrypt_t(const UINT8 *input, int inputOctets, UINT8 *outBuffer, bool isLast)
{
  int i, numBlocks, padLen;
  UINT8 block[16], *iv;
...
//      assert(padLen > 0 && padLen <= 16); // DO SOMETHING HERE ?
  ...
  if (inputOctets % 16 != 0)
	 return 16*(numBlocks + 1);
  else if (inputOctets % 16 == 0 && isLast)
	  return 16 * (numBlocks + 1);
  else
	 return 16 * numBlocks;
...
int Rijndael::padDecrypt_t(const UINT8 *input, int inputOctets, UINT8 *outBuffer, bool isLast)
{
  int i, numBlocks, padLen;
  UINT8 block[16];
  UINT32 iv[4];
