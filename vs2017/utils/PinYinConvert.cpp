...
#include <stdlib.h>
#include <memory.h>
	static std::string utf8_2_gbk(std::string  src) {
		std::string ans;
		int  len = src.length() * 2 + 1;
	...
	///比对结构体
	struct   tag_ch_zone{
		std::string   key;      //对应的字符
		std::string strB;     //开始的字符
		std::string strE;     //结束的字符
		bool   has_key(std::string & str) {
			if (str >= strB && str <= strE) {
				return true;
			}
			return false;
		}
		tag_ch_zone(std::string ch, std::string strb, std::string stre) {
			key = ch;
			strB = utf8_2_gbk(strb);
			strE = utf8_2_gbk(stre);
		}
	};
	#define MAX_CHAR_CNT 23
	///gbk 汉字编码范围
	static tag_ch_zone  g_ch_Array[MAX_CHAR_CNT] = {
		tag_ch_zone("A", "啊", "澳"),
		tag_ch_zone("B", "芭", "怖"),
		tag_ch_zone("C", "擦", "错"),
		tag_ch_zone("D", "搭", "堕"),
	...
	// 获取中文名称第一个字首字母
	static std::string get_chinese_firse_char(std::string strText) {
		if (strText.length() <= 1){
			return strText;
		}
		std::string strGbk = (strText);
		///判断第一个字是否函数
		unsigned char first_char = (unsigned char)strGbk[0];
		///非汉字，直接返回
		...
		std::string strSub = utf8_2_gbk(strGbk.substr(0, 3));
		for (int i = 0; i < MAX_CHAR_CNT; i++) {
			//std::transform (strSub.begin(),strSub.end(), strSub.begin(), ::toupper);
			if (g_ch_Array[i].has_key(strSub)) {
				return g_ch_Array[i].key;
			}
		}
		return strText;
	}
#endif
	int PinYinConvert::GetTeam(std::string name){
		std::string namePinYin("");
		int team = 0;
#ifdef _WIN32
		char szAnsi[260] = { 0 };
		utf82char((char*)name.c_str(), szAnsi);
		namePinYin = GetLetter(szAnsi);
#else 
		std::string test = utf8_2_gbk(name);
		namePinYin = GetLetter(test);
#endif
		if (namePinYin.size() > 0)
		{
			if (namePinYin.at(0) >= 65 && namePinYin.at(0) <= 90)
			{
				team = namePinYin.at(0);
			}
			else if (namePinYin.at(0) >= 97 && namePinYin.at(0) <= 122)
			{
				team = namePinYin.at(0) - 32;   //转换为大写
			}
			//#号组
			else
			{
	...
	std::string PinYinConvert::GetLetter(std::string strText)
	{
//#ifdef _WIN32
		...
		for (size_t i = 0; i < strText.length(); i++)
		{
			...
			if (!m_blnAllBiG)
			{
				for (size_t j = 1; j < strRes.length(); j++)
				{
...
//		return get_chinese_firse_char(strText);
//#endif
	}
	std::string PinYinConvert::GetPinyin(std::string name)
	{
		std::string namePinYin("");
		int team = 0;
#ifdef _WIN32
	...
	std::string PinYinConvert::findLetter(int nCode)
	{
		std::string strValue;
		switch (nCode)
		{
