...
struct resp_commonResult : public _sdkrespcmd_base 
{
	...
	struct st_userBean
	{
		...
		bool operator == (const st_userBean& rhs) const	{
			return (extend == rhs.extend && info == rhs.info  && entExtend == rhs.entExtend && accounts == rhs.accounts);
		}
	};
    ...
    */
	...
	struct st_tinyGroupMemberBean {
		...
		IS_SET(std::string, sector)		///< 界别 sector.
	...
	*/
	...
	{
		st_inviteCodeParam() :createUserID(0), digit(0), effectiveSeconds(0), effectiveCount(0){}
	...
	struct st_zxFavorite
	{
		...
		st_zxFavorite() :favoriteId(0), groupId(0), favoUserId(0), messageUserId(0), messageType(0), SDKID(0)
		{}
