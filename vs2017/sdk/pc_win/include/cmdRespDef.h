...
/**
* \struct  resp_zxSearchResult 消息号　logic_cmd_zxSearch
* \brief  政协定制查询接口
*/
struct resp_zxSearchResult : public _sdkrespcmd_base
{
    void init()
    {
        users.clear();
    ...
    int64 totalPage;
    std::vector<st_userBean>  users;

    CMD_SIZE();
    COPY_OTHER(resp_zxSearchResult);
};

/**
* \struct  resp_zxFavoSearchResult 消息号　logic_cmd_zxSearchFavo
* \brief  政协查询收藏接口
*/
struct resp_zxFavoSearchResult : public _sdkrespcmd_base
{
	void init()
	{
		favorites.clear();
	...
	int32 totalCount;
	int32 totalPage;
	std::vector<st_zxFavorite>  favorites;

	CMD_SIZE();
	COPY_OTHER(resp_zxFavoSearchResult);
};

struct resp_zxGetTopResult : public _sdkrespcmd_base
{
	void init()
	{
		tops.clear();
	...
	int32 totalCount;
	int32 totalPage;
	std::vector<st_zxTopInfo>  tops;

	CMD_SIZE();
	COPY_OTHER(resp_zxGetTopResult);
};
